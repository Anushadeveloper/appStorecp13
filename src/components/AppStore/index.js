...
class AppStore extends Component {
  ...
  getActiveTabs = () => {
    ...
    const finalListApp = appsList.filter(
  ...
  render() {
    ...
    const finalList = this.getActiveTabs()
    const searchInputList = finalList.filter(each =>
        ...
        <ul className="tab-list">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              onActiveTab={ActivetabId === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="app-list">
          {searchInputList.map(eachItem => (
